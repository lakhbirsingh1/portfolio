            ...
            <div className="grid grid-cols-2">
                {experienceSections.map((section, index) => (
                    <div key={index} className="p-8 pl-10 border-r border-b border-slate-700/50">
                        <h2 className="text-2xl text-white flex gap-2 mb-3">
                        ...
                        <div className="relative before:absolute before:top-4 before:rounded-full before:left-[-.3em] before:bg-gray-700 before:bottom:0 before:w-px before:h-full">
                            {section.items.map((item, itemIndex) => (
                                <div key={itemIndex} className="card flex flex-col items-start relative border border-slate-700/50 px-4 py-6 bg-gray-800/10 rounded-3xl mb-5 after:content-[''] after:absolute after:top-4 after:rounded-full after:left-[-.36em] after:bg-gray-500 first-of-type:after:bg-green-500 after:w-2 after:h-2 first-of-type:before:content-[''] first-of-type:before:absolute first-of-type:before:top-4 first-of-type:before:rounded-full first-of-type:before:left-[-.36em] first-of-type:before:bg-gray-500 first-of-type:before:bg-green-500 first-of-type:before:w-2 first-of-type:before:h-2 first-of-type:before:animate-ping">
                                    ...
                                    <h3 className="font-semibold text-xl text-white mb-1 mt-4">{item.title}</h3>
                                    <h6 className="text-sm text-white text-gray-400 mb-3">{item.company}</h6>
                ...
                {educationSections.map((section, index) => (
                    <div key={index} className="p-8 pl-10 border-r border-b border-slate-700/50">
                        ...
                        <div className="relative before:absolute before:top-4 before:rounded-full before:left-[-.3em] before:bg-gray-700 before:bottom:0 before:w-px before:h-full">
                            {section.items.map((item, itemIndex) => (
                                <div key={itemIndex} className="card flex flex-col items-start relative border border-slate-700/50 px-4 py-6 bg-gray-800/10 rounded-3xl mb-5 after:content-[''] after:absolute after:top-4 after:rounded-full after:left-[-.36em] after:bg-gray-500 after:w-2 after:h-2">
                                    <span className="text-sm mb-2 inline-block border border-gray-600 px-2 rounded text-gray-500">{item.tag}</span>
                                    <h3 className="font-semibold text-xl text-white mb-1 mt-4">{item.title}</h3>
                                    <h6 className="text-sm text-white text-gray-400 mb-3">{item.company}</h6>
                                    <p className="text-sm text-white">{item.description}</p>
                                </div>
